...
| 4   | **敏感数据不出本地**     | 使用 Skill 时不要把生产库中的 PII（身份证、手机号、银行卡号）粘贴到对话里。生成的 SQL 中如果包含字段级脱敏逻辑，执行前检查脱敏规则是否合规                                                    |
| 5   | **理解技术栈差异**      | 不保证生成的 SQL 或代码可以零修改直接在目标引擎运行。不同技术选型可能导致语法差异、性能特性、存储限制由使用者自行适配和验证                                                                 |
